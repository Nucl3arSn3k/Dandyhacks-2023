...
def model_run(
    ...
    [Chat]:
    {chat_history.map(lambda x: f"- [{x['from'].capitalize()}]: {x['msg']}\n")}
    ...
    From the questions you have asked the user and the responses generated by the user, give an assessment of the user's results. Keep the results in this format for your response:
    ...
    response = model.predict(prompt if not final_prompt else promptFinal, **parameters)
